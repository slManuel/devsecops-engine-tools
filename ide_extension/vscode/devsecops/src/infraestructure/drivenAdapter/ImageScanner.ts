...
export class ImageScanner implements IScannerGateway {
  scan(
    ...
    return new Promise((resolve, _reject) => {
      ...
      const dockerCommand = `${dockerPath} run --rm -v /var/run/docker.sock:/var/run/docker.sock juanca/devsecops-engine-tools:1.69.0 sh -c "devsecops-engine-tools --platform_devops local --remote_config_source local --remote_config_repo docker_default_remote_config --module engine_container --tool trivy --image_to_scan ${elementToScan} --context true"`;
