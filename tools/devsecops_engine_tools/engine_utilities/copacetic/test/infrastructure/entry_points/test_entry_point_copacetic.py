...
class TestEntryPointCopacetic(unittest.TestCase):
    ...
    @patch('devsecops_engine_tools.engine_utilities.copacetic.src.infrastructure.entry_points.entry_point_copacetic.MyLogger')
    @patch('devsecops_engine_tools.engine_utilities.copacetic.src.infrastructure.entry_points.entry_point_copacetic.logger')
    @patch('devsecops_engine_tools.engine_utilities.copacetic.src.infrastructure.entry_points.entry_point_copacetic.Copacetic')
    def test_remote_config_calls(self, mock_copacetic, mock_logger, mock_mylogger):
        """Test that remote config is called with correct parameters"""
        # Arrange
        mock_copacetic_instance = Mock()
        mock_input_core = Mock()
        mock_input_core.stage_pipeline = "patch"
        mock_copacetic_instance.process.return_value = mock_input_core
        mock_copacetic.return_value = mock_copacetic_instance
        ...
        # Assert
        expected_calls = [
            (self.args["remote_config_repo"], "/engine_core/ConfigTool.json", self.args["remote_config_branch"]),
            (self.args["remote_config_repo"], "/engine_integrations/copacetic/ConfigTool.json", self.args["remote_config_branch"]),
            (self.args["remote_config_repo"], "/engine_integrations/copacetic/Exclusions.json", self.args["remote_config_branch"])
        ]
        
        self.assertEqual(self.remote_config_source_gateway.get_remote_config.call_count, 3)
        for call_args in expected_calls:
            self.remote_config_source_gateway.get_remote_config.assert_any_call(*call_args)
    
    @patch('devsecops_engine_tools.engine_utilities.copacetic.src.infrastructure.entry_points.entry_point_copacetic.MyLogger')
    @patch('devsecops_engine_tools.engine_utilities.copacetic.src.infrastructure.entry_points.entry_point_copacetic.logger')
    def test_branch_validation_multiple_segments(self, mock_logger, mock_mylogger):
        """Test branch validation with multiple path segments"""
        # Arrange
        self.devops_platform_gateway.get_variable.side_effect = lambda var: {
            "pipeline_name": "test-pipeline",
        ...
        with patch('devsecops_engine_tools.engine_utilities.copacetic.src.infrastructure.entry_points.entry_point_copacetic.Copacetic') as mock_copacetic:
            mock_copacetic_instance = Mock()
            mock_input_core = Mock()
            mock_input_core.stage_pipeline = "patch"
            mock_copacetic_instance.process.return_value = mock_input_core
